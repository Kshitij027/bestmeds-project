...
router.post("/updatepassword", function (req, res) {
  bcrypt.hash(req.body.password, 12, function (hashError, passwordHash) {
    if (hashError) return res.status(500).json({ RESULT: false });
    pool.query(
    "update users set password=? where (mobileno=? or emailid=?)",
    [passwordHash, req.body.mobileno, req.body.mobileno],
    function (error, result) {
      if (error) {
        return res.status(500).json({ RESULT: false });
      } else {
...
router.post("/checkLogin", function (req, res) {
  pool.query(
    "select * from users where (mobileno=? or emailid=?)",
    [req.body.mobileno, req.body.mobileno],
    function (error, result) {
      if (error) {
        return res.status(500).json({ RESULT: false });
      }
      if (result.length === 0) return res.status(200).json({ RESULT: false });
      bcrypt.compare(req.body.password, result[0].password, function (compareError, matches) {
        if (compareError) return res.status(500).json({ RESULT: false });
        return res.status(200).json(matches ? { RESULT: true, data: result[0] } : { RESULT: false });
      });
    }
  );
});
...
router.post("/userdetails", function (req, res) {
  pool.query(
    ...
    [req.body.mobileno],
    function (error, result) {
      if (error) return res.status(500).json({ RESULT: false });
      if (result.length === 0) return res.status(200).json({ RESULT: false });
      bcrypt.compare(req.body.password, result[0].password, function (compareError, matches) {
        if (compareError) return res.status(500).json({ RESULT: false });
        return res.status(200).json(matches ? { RESULT: true, data: result } : { RESULT: false });
      });
    }
  );
});
...
router.post("/changepassword", function (req, res) {
  ...
  bcrypt.hash(req.body.password, 12, function (hashError, passwordHash) {
    ...
    [passwordHash, req.body.mobileno],
    function (error, result) {
      if (error) {
        return res.status(500).json([]);
      } else {

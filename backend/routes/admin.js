...
router.post('/checkadminlogin', function(req, res, next) {
    pool.query("select * from administrator where emailid=?", [req.body.emailid], async function(error, result) {
      if (error) return res.status(500).json({ result: false });
      if (result.length === 0) return res.status(200).json({ result: false });

      try {
        const matches = await bcrypt.compare(req.body.password, result[0].password);

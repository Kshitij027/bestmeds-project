...
router.post('/displayallbrandsbystatus', function (req, res) {
  console.log('Body in /brands/displayallbrandsbystatus:', req.body);

  // In Home.js you call with: { status: 'Popular' }
  pool.query(
    'SELECT * FROM brands WHERE status = ?',
    [req.body.status],
    function (err, result) {
      if (err) {
        console.log('Error in /brands/displayallbrandsbystatus:', err);
        return res
          .status(500)

...
router.post('/displaydevicebystatus', function (req, res) {
  console.log('Body in /products/displaydevicebystatus:', req.body);
  // you send: { status: 'Trending', categoryid: 9 } from Home.js

  pool.query(
    'SELECT * FROM products WHERE status = ? AND categoryid = ?',
    [req.body.status, req.body.categoryid],
    function (err, result) {
      if (err) {
        console.log('Error in /products/displaydevicebystatus:', err);
        return res
          .status(500)
      ...
      return res
        .status(200)
        .json({ status: true, result: result, data: result });
    }
  );
});
...
router.post("/displayallproductsbybrandid", function (req, res, next) {
  ...
  pool.query(
    "select P.*,(select C.categoryname from categories C where C.categoryid=P.categoryid)as categoryname,(select S.subcategoryname from subcategories S where S.subcategoryid=P.subcategoryid)as subcategoryname,(select B.brandname from brands B where B.brandid=P.brandid)as brandname from products P where P.brandid=?", [req.body.brandid],
    function (error, result) {
      if (error) {
        console.log(error)
        res.status(500).json({ result: [] });
...
router.post("/displayallproductsbysubcategoryid", function (req, res, next) {
  console.log(req.body.brandid)
  pool.query(
    "select P.*,(select C.categoryname from categories C where C.categoryid=P.categoryid)as categoryname,(select S.subcategoryname from subcategories S where S.subcategoryid=P.subcategoryid)as subcategoryname,(select B.brandname from brands B where B.brandid=P.brandid)as brandname from products P where P.subcategoryid=?", [req.body.subcategoryid],
    function (error, result) {
      if (error) {
        console.log(error)
        res.status(500).json({ result: [] });

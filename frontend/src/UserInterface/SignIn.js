...
export default function CustomerLogin() {
  ...
  const handleChkOtp = () => {
    if (OTP == gOtp) {
      localStorage.setItem('user', JSON.stringify({ [data.mobileno]: data }))
      navigate('/showcart')
    }
  ...
  const handleSubmit = async () => {
    if (ROTP == rgOtp) {
      ...
      if (result && result.result) {
        ...
        localStorage.setItem('user', JSON.stringify({ [mobileno]: body }))
        navigate('/cartreview')
      } else {
          ...
          <Grid item xs={12} md={6}>
            <div className="main">
              ...
              <div className="login">
                ...
                { }
                <input type="text" value={firstname} onChange={(event) => setFirstName(event.target.value)} name="txt" placeholder="First Name" required="" style={{ margin: '10px auto 10px auto' }} />
                <input type="text" value={lastname} onChange={(event) => setLastName(event.target.value)} name="txt" placeholder="Last Name" required="" style={{ margin: '10px auto 10px auto' }} />
                <input type="email" value={emailid} onChange={(event) => setEmailId(event.target.value)} name="email" placeholder="Email" required="" style={{ margin: '10px auto 10px auto' }} />

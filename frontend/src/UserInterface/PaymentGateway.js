...
const PaymentGateway = (props) => {
  // Razorpay is not integrated yet: production checkout must create an order and
  // verify the payment signature on the backend before an order is marked paid.
...
var dispatch=useDispatch()
  ...
  const OnlineMethod = () => {
    // Not yet integrated: create and verify Razorpay orders on the backend before
    // re-enabling this method. Client-side payment IDs are not proof of payment.
    setMsg('Online payments are temporarily unavailable. Please choose cash on delivery.');
    setStatus(true);
  }
